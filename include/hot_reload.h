...
namespace hot_reload {
  ...
  template <typename E, size_t NumSymbols> class Module {
  ...
  protected:
    ...
    template <size_t Index, typename Ret, typename... Args> Ret Execute(Args... args) {
    ...
    template <size_t Index, typename T> T* GetVar() {
      // Prevent index out of bounds at compile-time
      static_assert(Index >= 0 && Index < NumSymbols, "Out of bounds symbol index");
